...
Welcome to the **CSA Relevance Engine** package. This Python library provides a suite of tools and functions for performing relevance-based predictions using the Cambridge Sports Analytics (CSA) API. The package is designed to facilitate single and multi-task predictions, allowing for flexible model evaluation and experimentation.
...
```
csanalytics/
    ├── relevance_engine.py         # Main module for user interactions
    ├── bin/                        # Internal modules for task management
    │   ├── multi_tasks.py          # Handles multi-y and multi-theta tasks
    ...
    │   └── threaded_predictions.py # Handles predictions in a multi-threaded environment
...
To install the CSAnalytics Relevance Engine package, use the following command:

```bash
pip install csanalytics
```
...
We welcome contributions to the CSA Relevance Engine package. If you find a bug or have a feature request, please reach out to the CSA support team.

## License

(c) 2023 - 2024 Cambridge Sports Analytics, LLC. All rights reserved. For support, please contact: support@csanalytics.io